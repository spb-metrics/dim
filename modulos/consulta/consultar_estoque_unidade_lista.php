...
// | Arquivo ............: consultar_estoque_unidade_lista.php                       |
...
function atualiza()
{
  document.form_busca.action = '<? echo $PHP_SELF; ?>?codigo=<?=$_GET['codigo']?>&aplicacao=<?=$_GET['aplicacao']?>&unidade=<?=$_GET['unidade']?>';
      ...
      ?>
                      <?php
                        if(isset($_GET[unidade])){
                          $sql="select nome from unidade where id_unidade='$_GET[unidade]' and status_2='A'";
                          $res=mysqli_query($db, $sql);
                          erro_sql("Unidade Selecionada", $db, "");
                          if(mysqli_num_rows($res)>0){
                            $unidade_info=mysqli_fetch_object($res);
                          }
                        }
                      ?>
                      <td valign="center" width="25%">Unidade
                        <input type="text" name="unidade" style="width: 150px" value="<?php echo $unidade_info->nome;?>" disabled>
                      </td>
                      <td valign="center" width="55%">Material
                        <input type="hidden" name="cod_mat" id="cod_mat" style="width: 70px" value="<?=$cod_mat?>" disabled>
                        <input type="text" name="desc_mat" id="desc_mat" style="width: 300px" value="<?=$desc_mat?>" disabled>
                      </td>
                      <td valign="center" width="100%" height="21">Ordenar
                        <select size="1" name="indice" style="width: 75px" onChange="atualiza();">
...
<?
                      ...
                      ////////////////////////////////////////
                      echo "<a href='".URL."/modulos/consulta/consultar_estoque_unidade_lista.php?pagina=0&pagina_a_exibir=1&codigo=$codigo&aplicacao=$aplicacao&unidade=$unidade&indice=".$_POST['indice']."'>
...
<?php
                      ...
                      if($pagina_exibicao==1){
                        ...
                        echo "<a href='".URL."/modulos/consulta/consultar_estoque_unidade_lista.php?pagina=" . ($pagina_anterior-1) . "&pagina_a_exibir=$pagina_anterior&codigo=$codigo&aplicacao=$aplicacao&unidade=$unidade&indice=".$_POST['indice']."'>
...
<?php
                     ...
                     if($pagina_exibicao==$total_paginas){
                       ...
                       echo "<a href='".URL."/modulos/consulta/consultar_estoque_unidade_lista.php?pagina=" . ($proxima_pagina-1) . "&pagina_a_exibir=$proxima_pagina&codigo=$codigo&aplicacao=$aplicacao&unidade=$unidade&indice=".$_POST['indice']."'>
...
?>
     ...
     <tr>
       <td colspan="2" align="right" class="descricao_campo_tabela">
         <input style="font-size: 12px;" type="button" name="voltar"  value="<< Voltar"  onClick="window.location='<?php echo URL;?>/modulos/consulta/consulta_estoque_unidade.php?pagina=<?=$_GET[paginav]?>&pagina_a_exibir=<?=$_GET[pagina_a_exibirv]?>&aplicacao=<?=$aplicacao?>&material=<?php echo $codigo;?>&indice=<?php echo $_GET['indice'];?>'">
       </td>
     </tr>
   </table>
<?php
