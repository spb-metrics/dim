...
*/
  ...
  if (file_exists("../../config/config.inc.php"))
  {
    ...
    $max_tentativas = $parametro->tentativas_senha;
    if ($max_tentativas=='')
    {
     $max_tentativas = 3;
    }
    ...
    $id_itens_receita = $_GET['id_itens_receita'];
...
function preencheCampos(id)
{
	if (window.showModalDialog)
	{
        var _R = new Object()
		_R.id=id;
		_R.material= document.form_autoriza.material.value;
	...
	else
	{
		...
		{
			window.opener.SetNameAutorizador(id, document.form_autoriza.id_itens_receita.value, document.form_autoriza.material.value);
		}
	}
	window.close();
}
...
<body onload='document.form_autoriza.login.focus()'>
    <form name="form_autoriza" action="autorizador_alteracao.php" method="POST" enctype="application/x-www-form-urlencoded">
          <table bgcolor="#FFFFFF" align='center' border="0" cellspacing="1" cellpadding="0" width="100%">
          <input type="hidden" size="20" name="material" value="<?php echo $material; ?>">
          <input type="hidden" name="id_itens_receita" id="id_itens_receita" value="<?php echo $id_itens_receita;?>">
