...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: grupo_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de grupos
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          {
             $string_query_registros = "select id_grupo, descricao from grupo
          ...
          else
          {
            ...
            $string_query_registros = "select id_grupo, descricao from grupo where status_2='A' order by ".$_POST['indice'];
          }
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./grupo_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" id="pesquisa" style="width: 150px">
                            <option value="id_grupo" <?php if($_POST[pesquisa]=="id_grupo"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Grupo </option>
                        </select>
                      ...
                      <td valign="middle" width="40%">
                        ...
                        <select name="indice" id="indice" style="width: 150px">
                            <option value="id_grupo" <?php if($_POST[indice]=="id_grupo"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Grupo </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/grupo/grupo_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/grupo/grupo_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
...
?>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 ...
                 <td align='center'>
                   ...
                   if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/grupo/grupo_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_grupo;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/grupo/grupo_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_grupo;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/grupo/grupo_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_grupo;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/grupo/grupo_inicial.php";
...
//echo"Valor de e:".$_GET[e];
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o grupo!')</script>";}
  
    if($_GET[i]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o grupo!')</script>";}
  
    if($_GET[a]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o grupo!')</script>";}
  }
  ////////////////////////////////////////////
