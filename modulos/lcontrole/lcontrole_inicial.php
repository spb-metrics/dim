...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: lcontrole_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de listas de controle
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          {
             $string_query_registros = "select id_lista_especial, lista, descricao from lista_especial
          ...
          else
          {
            ...
            if ($_GET['pesquisa']!='')
            {
              $string_query_registros = "select id_lista_especial, lista, descricao from lista_especial
               ...
               $string_query_registros = "select id_lista_especial, lista, descricao from lista_especial where status_2='A' order by ".$_POST['indice'];
            }
          }
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./lcontrole_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" style="width: 150px">
                            <option value="lista" <?php if($_POST[pesquisa]=="lista"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Lista de Controle Especial </option>
                        </select>
                      ...
                      <td valign="middle" width="40%">
                        ...
                        <select name="indice" style="width: 150px">
                            <option value="lista" <?php if($_POST[indice]=="lista"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Lista de Controle Especial </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/lcontrole/lcontrole_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/lcontrole/lcontrole_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
...
?>
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 ...
                 <td align='center'>
                   ...
                   if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/lcontrole/lcontrole_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $lista_info->id_lista_especial;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/lcontrole/lcontrole_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $lista_info->id_lista_especial;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/lcontrole/lcontrole_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $lista_info->id_lista_especial;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/lcontrole/lcontrole_inicial.php";
...
<?php
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir a lista!')</script>";}
  
    if($_GET[i]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar a lista!')</script>";}

    if($_GET[a]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar a lista!')</script>";}
  }
  ////////////////////////////////////////////
