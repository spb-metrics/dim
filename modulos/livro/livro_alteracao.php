...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: livro_alteracao.php
  //  Bancos...: dbtdim
  //  Data.....: 28/11/2007
  //  Analista.: Ricieri Rocha Conz
  //  Função...: Tela de alteração do Livro
  //////////////////////////////////////////////////////////////////
  ...
  if (file_exists("../../config/config.inc.php"))
  {
    ...
    if(isset($_POST[descricao])){
      ...
      $sql="update livro ";
      $sql.="set descricao='" . strtoupper($_POST[descricao]) . "', status_2='A', data_alt='$data', usua_alt='$_SESSION[id_usuario_sistema]' ";
      $sql.="where id_livro='$_POST[codigo_antigo]'";
      mysqli_query($db, $sql);
      erro_sql("Update Alteração Livro", $db, "");
      ...
      if(mysqli_errno($db)=="0")
      {
        mysqli_commit($db);
        $aux=$_POST[aux];
        header("Location: ". URL."/modulos/livro/livro_inicial.php?a=t&".$aux);
      }
      else
      {
        mysqli_rollback($db);
        header("Location: ". URL."/modulos/livro/livro_inicial.php?a=f");
      }
    }
    else{
      if($_GET[codigo]==""){
        header("Location: ". URL."/modulos/livro/livro_inicial.php");
      }
      else{
        $sql="select id_livro, descricao from livro where id_livro='$_GET[codigo]'";
        $res=mysqli_query($db, $sql);
        erro_sql("Select Alteração Livro", $db, "");
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="1" border="0">
                  <form name="form_alteracao" action="./livro_alteracao.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr>
                      ...
                      <td class="campo_tabela" colspan="2" valign="middle" width="100%">
                        <input type="text" name="codigo" size="30" style="width: 200px" disabled value="<?php echo $grupo_info->id_livro?>">
                      </td>
                    </tr>
                    <tr>
                      <td class="descricao_campo_tabela" valign="middle" width="20%">
                      ...
                      <td class="campo_tabela" colspan="2" valign="middle" width="100%">
                        <input type="text" name="descricao" size="60" style="width: 520px" value="<?php echo $grupo_info->descricao?>">
                      </td>
                    </tr>
                    <tr class="campo_botao_tabela" height="35">
                      <td colspan="3" valign="middle" align="right" width="100%">
                        <input type="button" style="font-size: 12px;" name="voltar" value="<< Voltar" onclick="window.location='<?php echo URL;?>/modulos/livro/livro_inicial.php?pagina=<?=$_GET[pagina]?>&pagina_a_exibir=<?=$_GET[pagina_a_exibir]?>&buscar=<?=$_GET[buscar]?>&indice=<?=$_GET[indice]?>&pesquisa=<?=$_GET[pesquisa]?>'">
