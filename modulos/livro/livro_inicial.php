...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: livro_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 128/11/2007
  //  Analista.: Ricieri Rocha Conz
  //  Função...: Tela inicial do Livro
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          {
             $string_query_registros = "select id_livro, descricao from livro
          ...
          else
          {
            ...
            $string_query_registros = "select id_livro, descricao from livro where status_2='A' order by ".$_POST['indice'];
          }
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./livro_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" style="width: 150px">
                          <option value="id_livro" <?php if($_POST[pesquisa]=="id_livro"){echo "selected";}?>> Código </option>
                          <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Livro </option>
                        </select>
                      ...
                      <td valign="middle" width="40%">
                        ...
                        <select name="indice" style="width: 150px">
                          <option value="id_livro" <?php if($_POST[indice]=="id_livro"){echo "selected";}?>> Código </option>
                          <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Livro </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/livro/livro_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/livro/livro_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 ...
                 <td align='center'>
                   <?php
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/livro/livro_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_livro;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/livro/livro_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_livro;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/livro/livro_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_livro;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/livro/livro_inicial.php";
...
<?php
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o Livro!')</script>";}
    if($_GET[e]=='r'){echo "<script>window.alert('Não é possícel excluir o livro, pois existe lista de controle especial associado')</script>";}
    
    if($_GET[i]=='t'){echo "<script>window.alert('Operação realizada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o Livro!')</script>";}
  
    if($_GET[a]=='t'){echo "<script>window.alert('Operação realizada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o Livro!')</script>";}
  }
  ////////////////////////////////////////////
