...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: material_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de materiais
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          {
            if ($_POST['pesquisa']=='flg_dispensavel')
              $_POST['buscar'] = substr($_POST['buscar'],0,1);
              $string_query_registros = "select codigo_material, descricao, flg_dispensavel, id_material
                                         from material
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./material_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      ...
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" id="pesquisa" style="width: 150px">
                            ...
                            <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Material </option>
                            <option value="flg_dispensavel" <?php if($_POST[pesquisa]=="flg_dispensavel"){echo "selected";}?>> Dispensável </option>
                        </select>
                      ...
                      <td valign="middle" width="40%">
                        Ordenar Lista:
                        <select name="indice" id="indice" style="width: 150px">
                            <option value="codigo_material" <?php if($_POST[indice]=="codigo_material"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Material </option>
                            <option value="flg_dispensavel" <?php if($_POST[indice]=="flg_dispensavel"){echo "selected";}?>> Dispensável </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/material/material_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/material/material_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
...
<?php
            ...
            while ($material_info = mysqli_fetch_object($resultado_query_limite))
            {
?>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 <td align='left'>
                    <?php echo $material_info->codigo_material;?>
                 </td>
                 <td align='left'>
                    <?php echo $material_info->descricao;?>
                 </td>
                 <td align='center'>
                    <?php
                      if ($material_info->flg_dispensavel == 'S')
                        echo "SIM";
                      else echo "NÃO";
                    ?>
                 </td>
                 <td align='center'>
                   <?php
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/material/material_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $material_info->id_material;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/material/material_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $material_info->id_material;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/material/material_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $material_info->id_material;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/material/material_inicial.php";
...
<?php
    ...
    if($_GET[e]=='t'){echo "<script>window.alert('Material excluído com sucesso!')</script>";}
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o material!')</script>";}
  
    if($_GET[i]=='t'){echo "<script>window.alert('Material cadastrado com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o material!')</script>";}
  
    if($_GET[a]=='t'){echo "<script>window.alert('Material alterado com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o material!')</script>";}
  }
  ////////////////////////////////////////////
