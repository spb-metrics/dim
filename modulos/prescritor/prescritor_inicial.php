...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: prescritor_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de prescritores
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          if ($_POST['pesquisa']!='' and $_POST['indice']!='' and $_POST['buscar']!='')
          {
            if($_POST[indice]=="descricao"){
              $_POST[indice]="p.descricao";
            }
            if($_POST[indice]=="conselho_descricao"){
              $_POST[indice]="c.descricao";
            }
            
            if($_POST[pesquisa]=="descricao"){
              $_POST[pesquisa]="p.descricao";
            }
            if($_POST[pesquisa]=="conselho_descricao"){
              $_POST[pesquisa]="c.descricao";
            }
            
             $string_query_registros = "select p.id_tipo_prescritor, p.descricao as pdescricao, c.descricao as cdescricao
                                        from tipo_prescritor as p, tipo_conselho as c
                                        where p.tipo_conselho_id_tipo_conselho=c.id_tipo_conselho and
                                        ".$_POST['pesquisa']." like '%".trim($_POST['buscar'])."%' and p.status_2='A'
                                        order by ".$_POST['indice'];
            if($_POST[indice]=="p.descricao"){
              $_POST[indice]="descricao";
            }
            if($_POST[indice]=="c.descricao"){
              $_POST[indice]="conselho_descricao";
            }
            
            if($_POST[pesquisa]=="p.descricao"){
              $_POST[pesquisa]="descricao";
            }
            if($_POST[pesquisa]=="c.descricao"){
              $_POST[pesquisa]="conselho_descricao";
            }
          ...
          else
          {
            ...
            $string_query_registros = "select p.id_tipo_prescritor, p.descricao as pdescricao, c.descricao as cdescricao
                                       from tipo_prescritor as p, tipo_conselho as c
                                       where p.tipo_conselho_id_tipo_conselho=c.id_tipo_conselho and
                                       p.status_2='A' order by ".$_POST['indice'];
            if($_POST[indice]=="p.descricao"){
              $_POST[indice]="descricao";
            }
            if($_POST[indice]=="c.descricao"){
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./prescritor_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      ...
                      <td valign="middle" width="60%">
                        Pesquisar por:
                        <select name="pesquisa" id="pesquisa" style="width: 150px">
                            <option value="id_tipo_prescritor" <?php if($_POST[pesquisa]=="id_tipo_prescritor"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Profissional </option>
                            <option value="conselho_descricao" <?php if($_POST[pesquisa]=="conselho_descricao"){echo "selected";}?>> Conselho Profissional </option>
                        </select>
                      ...
                      <td valign="middle" width="40%">
                        ...
                        <select name="indice" style="width: 150px">
                            <option value="id_tipo_prescritor" <?php if($_POST[indice]=="id_tipo_prescritor"){echo "selected";}?>> Código </option>
                            <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Profissional </option>
                            <option value="conselho_descricao" <?php if($_POST[indice]=="conselho_descricao"){echo "selected";}?>> Conselho Profissional </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/prescritor/prescritor_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/prescritor/prescritor_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
...
<?php
            ...
            while ($prescritor_info = mysqli_fetch_object($resultado_query_limite))
            {
?>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 <td align='left'>
                    <?php echo $prescritor_info->id_tipo_prescritor;?>
                 </td>

                 <td align='left'>
                    <?php echo $prescritor_info->pdescricao;?>
                 </td>
                 <td align='left'>
                    <?php echo $prescritor_info->cdescricao;?>
                 </td>
                 <td align='center'>
                   <?php
                     if ($pagina_a_exibir=="")
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/prescritor/prescritor_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $prescritor_info->id_tipo_prescritor;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/prescritor/prescritor_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $prescritor_info->id_tipo_prescritor;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/prescritor/prescritor_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $prescritor_info->id_tipo_prescritor;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/prescritor/prescritor_inicial.php";
...
//echo "valor de i".$_GET[i];
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o tipo de conselho!')</script>";}
  
    if($_GET[i]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o tipo de conselho!')</script>";}
  
    if($_GET[a]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o tipo de conselho!')</script>";}
  }
  ////////////////////////////////////////////
