...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: profissional_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de profissionais
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          if($_POST['busca'] == '')
          {
            ...
            {
               $string_query_registros = "select p.id_profissional, p.inscricao, c.descricao,
                                                 p.nome from profissional as p,
                                                 tipo_conselho as c
                                          where ".$_POST['pesquisa']." like '".trim($_POST['buscar'])."%'
                                                 and p.status_2='A'
                                                 and p.tipo_conselho_id_tipo_conselho = c.id_tipo_conselho
            ...
            else
            {
              ...
              $string_query_registros = "select p.id_profissional, p.inscricao, c.descricao,
                                         ...
                                         where p.status_2='A'
                                               and p.tipo_conselho_id_tipo_conselho = c.id_tipo_conselho
                                               and p.cidade_id_cidade = pr.cidade_id_cidade
                                         order by ".$_POST['indice'];
            }
          }
          else
          {
            if ($_POST['indice']!='' and $_POST['pesquisar']!='' and $_POST['buscar']!='')
            {
               $string_query_registros = "select p.id_profissional, p.inscricao, c.descricao,
                                                 p.nome from profissional as p,
                                                 tipo_conselho as c
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./profissional_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" id="pesquisa" style="width: 150px">
                            <option value="inscricao" <?php if($_POST[pesquisa]=="inscricao"){echo "selected";}?>> Inscrição </option>
                            <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Conselho Profissional </option>
                            <option value="nome" <?php if($_POST[pesquisa]=="nome"){echo "selected";}?>> Nome </option>
                        </select>
                        
                        <input type="text" name="buscar" id="buscar" size="30" style="width: 200px" <?php if (isset($_POST[buscar])){echo "value='".$_POST[buscar]."'";}?>>
                        <input type="hidden" name="busca" value="busca">
                      </td>
                      <td valign="middle" width="40%">
                        Ordenar Lista:
                        <select name="indice" id="indice" style="width: 150px">
                            <option value="inscricao" <?php if($_POST[indice]=="inscricao"){echo "selected";}?>> Inscrição </option>
                            <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Conselho Profissional </option>
                            <option value="nome" <?php if($_POST[indice]=="nome"){echo "selected";}?>> Nome </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        ...
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/profissional/profissional_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/profissional/profissional_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
...
<?php
            ...
            while ($consulta_info = mysqli_fetch_object($resultado_query_limite))
            {
?>
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 <td align='left'>
                   <?php echo $consulta_info->inscricao;?>
                 </td>
                 <td align='left'>
                   <?php echo $consulta_info->descricao;?>
                 </td>
                 <td align='left'>
                    <?php echo $consulta_info->nome;?>
                 </td>
                 <td align='center'>
                   <?php
                   if ($pagina_a_exibir=="")
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/profissional/profissional_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $consulta_info->id_profissional;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/profissional/profissional_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $consulta_info->id_profissional;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/profissional/profissional_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $consulta_info->id_profissional;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/profissional/profissional_inicial.php";
...
<?php
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o profissional!')</script>";}
  
    if($_GET[i]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o profissional!')</script>";}

    if($_GET[a]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o profissional!')</script>";}
  }
  ////////////////////////////////////////////
