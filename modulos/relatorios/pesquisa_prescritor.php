...
// +---------------------------------------------------------------------------------+
  ...
  if (file_exists("../../config/config.inc.php"))
  {
    ...
    if(isset($_POST[pesquisar]) && $_POST[pesquisar]!=""){
      $sql="select id_profissional, inscricao, nome
            from profissional
            where status_2 = 'A'
                  and nome like '%" . trim($_POST[pesquisar]) . "%'
            order by nome";
...
?>
                <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                  ...
                  <td align="center">
                    <input type="radio" name="selecao" onclick="preencheCampos('<?php echo $consulta->id_profissional;?>', '<?php echo $consulta->nome;?>');window.close();">
                  </td>
                </tr>
<?php
                ////////////////////////
