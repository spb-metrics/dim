...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: remanejamento_inicial_fornec.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de remanejamento - fornecimento
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          if ($_POST['pesquisa']!='' and $_POST['indice']!='' and $_POST['buscar']!='')
          {
            ...
            $string_query_registros = "select sol.data_incl, u.nome, uni.nome as unidnome,
                                       ...
                                       sol.id_unid_solicitada='$_SESSION[id_unidade_sistema]'
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./remanejamento_inicial_fornec.php?aplicacao=<?php echo $_SESSION[APLICACAO];?>" method="POST" enctype="application/x-www-form-urlencoded">
                    <tr class="titulo_tabela">
                      <td colspan="3" valign="middle" align="center" width="100%" height="21"> <?php echo $nome_aplicacao;?> </td>
                    </tr>
                    <tr class="opcao_tabela">
                      <td valign="middle" colspan="2" width="60%">
                        Pesquisar por:
                        <select name="pesquisa" style="width: 150px" onchange="if(this.selectedIndex==0){document.form_inicial.buscar.value='';}">
                            <option value="data_incl" <?php if($_POST[pesquisa]=="data_incl"){echo "selected";}?>> Data Solicitação </option>
                            ...
                            <option value="id_solicita_remanej" <?php if($_POST[pesquisa]=="id_solicita_remanej"){echo "selected";}?>> Nº da Solicitação </option>
                        </select>
                        <input type="text" name="buscar" size="30" style="width: 200px" <?php if (isset($_POST[buscar])){echo "value='".$_POST[buscar]."'";}?> onblur="if(document.form_inicial.pesquisa.selectedIndex==0){verificaData(this,this.value);}" onKeyPress="if(document.form_inicial.pesquisa.selectedIndex==0){return mascara_data(event,this);}"  onkeypress=" return VerificarEnter(event);">
                      </td>
                      <td valign="middle" width="40%">
                        Ordenar Lista:
                        <select name="indice" style="width: 150px">
                            <option value="data_incl" <?php if($_POST[indice]=="data_incl"){echo "selected";}?>> Data Solicitação </option>
                            ...
                            <option value="id_solicita_remanej" <?php if($_POST[indice]=="id_solicita_remanej"){echo "selected";}?>> Nº da Solicitação </option>
                        </select>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 ...
                 <td align='center'>
                   ...
                   <?php
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/remanejamento/remanejamento_detalhado_fornec.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $solicitacao_info->id_solicita_remanej;?>&aplicacao=<?php echo $_GET[aplicacao];?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($inclusao_perfil!=""){
                   ?>
                     <a <?php if($status_solicitada!=""){echo "href='" . URL ."/modulos/remanejamento/remanejamento_atendido_fornec.php?responsavel=$mostrar_responsavel_dispensacao&codigo=" .$solicitacao_info->id_solicita_remanej . "&aplicacao=$_GET[aplicacao]'";}?>><?php if($status_solicitada!=""){echo "<img src='" . URL . "/imagens/b_edit.gif' border='0' title='Atender Registro'>";}else{echo "<img src='" . URL . "/imagens/gray/b_edit.png' border='0' title='Atender Registro'>";}?></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/remanejamento/remanejamento_inicial_fornec.php";
...
<?php
    ...
    ///////////////////////////////////////////////
    if($_GET[n]=='f'){echo "<script>window.alert('Solicitação não atendida!')</script>";}
    ...
    if($_GET[a]=='t'){
		echo "<script>resposta=window.confirm('Operação efetuada com sucesso! Deseja imprimir o recibo?');
				if(resposta){
					window.open('".URL."/modulos/impressao/impressao_remanejamento.php?codigo=$_GET[codigo]&unidade_solicitante=$_GET[unidade_solicitante]&unidade_solicitada=$_GET[unidade_solicitada]&chave=$_GET[chave]');
				}
			</script>";}
	
	
	
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível realizar o atendimento do remanejamento!')</script>";}
