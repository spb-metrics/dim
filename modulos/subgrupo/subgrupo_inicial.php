...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: subgrupo_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 27/11/2006
  //  Analista.: Fabio Hitoshi Ide
  //  Função...: Tela inicial do módulo de subgrupos
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          if ($_POST['pesquisa']!='' and $_POST['indice']!='' and $_POST['buscar']!='')
          {
            ...
            $string_query_registros = "select g.descricao as gdescricao, sbg.id_subgrupo, sbg.descricao
                                       from grupo as g, subgrupo as sbg
                                       where ".$_POST['pesquisa']." like '%".trim($_POST['buscar'])."%' and g.id_grupo=sbg.grupo_id_grupo and sbg.status_2='A'
          ...
          else
          {
            ...
            $string_query_registros = "select g.descricao as gdescricao, sbg.id_subgrupo, sbg.descricao
                                       from grupo as g, subgrupo as sbg
                                       where g.id_grupo=sbg.grupo_id_grupo and sbg.status_2='A'
                                       order by ".$_POST['indice'];
            if($_POST[indice]=="sbg.descricao"){
              $_POST[indice]="descricao";
            }
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./subgrupo_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" id="pesquisa" style="width: 150px">
                          <option value="grupo_descricao" <?php if($_POST[pesquisa]=="grupo_id_grupo"){echo "selected";}?>> Grupo </option>
                          <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Sub - Grupo </option>
                        </select>
                        <input type="text" name="buscar" id="buscar" size="30" style="width: 200px" value="<?php if(isset($_POST[buscar])){echo "$_POST[buscar]";}?>" onkeypress="VerificarEnter(event);">

                      </td>
                      <td valign="middle" width="40%">
                        Ordenar Lista:
                        <select name="indice" id="indice" style="width: 150px">
                          <option value="grupo_descricao" <?php if($_POST[indice]=="grupo_id_grupo"){echo "selected";}?>> Grupo </option>
                          <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Sub - Grupo </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/subgrupo/subgrupo_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/subgrupo/subgrupo_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 ...
                 <td align='center'>
                   <?php
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/subgrupo/subgrupo_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $sbgrupo_info->id_subgrupo?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/subgrupo/subgrupo_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $sbgrupo_info->id_subgrupo?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/subgrupo/subgrupo_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $sbgrupo_info->id_subgrupo?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/subgrupo/subgrupo_inicial.php";
...
<?php
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o subgrupo!')</script>";}
  
    if($_GET[i]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o subgrupo!')</script>";}
  
    if($_GET[a]=='t'){echo "<script>window.alert('Operação efetuada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o subgrupo!')</script>";}
  }
  ////////////////////////////////////////////
