...
*/
  ...
  /////////////////////////////////////////////////////////////////
  //  Sistema..: DIM
  //  Arquivo..: tmovimento_inicial.php
  //  Bancos...: dbtdim
  //  Data.....: 26/12/2007
  //  Analista.: Ricieri Rocha Conz
  //  Função...: Tela inicial Tipo Movimento
  //////////////////////////////////////////////////////////////////
...
<?
          ...
          {
             $string_query_registros = "select id_tipo_movto, descricao, operacao from tipo_movto
                                        where status_2='A' and ".$_POST['pesquisa']." like '%".trim($_POST['buscar'])."%'                                     order by ".$_POST['indice'];
          }
          ///////////////////////////////////////////////////
          ...
          else
          {
            ...
            $string_query_registros = "select id_tipo_movto, descricao, operacao from tipo_movto where status_2='A' order by ".$_POST['indice'];
          }
            ...
            <tr>
              <td colspan='8'>
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <form name="form_inicial" action="./tmovimento_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
                    ...
                    <tr class="opcao_tabela">
                      <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" style="width: 150px">
                          <option value="id_tipo_movto" <?php if($_POST[pesquisa]=="id_tipo_movto"){echo "selected";}?>> Código </option>
                          <option value="descricao" <?php if($_POST[pesquisa]=="descricao"){echo "selected";}?>> Movimento </option>
                          <option value="operacao" <?php if($_POST[pesquisa]=="operacao"){echo "selected";}?>> Operação </option>
                        </select>
                      ...
                      <td valign="middle" width="40%">
                        ...
                        <select name="indice" style="width: 150px">
                          <option value="id_tipo_movto" <?php if($_POST[indice]=="id_tipo_movto"){echo "selected";}?>> Código </option>
                          <option value="descricao" <?php if($_POST[indice]=="descricao"){echo "selected";}?>> Movimento </option>
                          <option value="operacao" <?php if($_POST[indice]=="operacao"){echo "selected";}?>> Operação </option>
                        </select>
                      ...
                      <td valign="middle" align="right" width="20%">
                        <?php
                          if($inclusao_perfil!=""){
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/tmovimento/tmovimento_inclusao.php'">
                        <?php
                          }
                          else{
                        ?>
                            <input type="button" style="font-size: 12px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/tmovimento/tmovimento_inclusao.php'" disabled>
                        <?php
                          }
                        ?>
                      </td>
...
?>
               ...
               <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D4DFED';" onMouseOut="this.bgColor='<?php echo $cor_linha;?>'">
                 <td align='left'>
                    <?php echo $grupo_info->id_tipo_movto;?>
                 </td>

                 <td align='left'>
                    <?php echo strtoupper($grupo_info->descricao);?>
                 </td>
                 
                 <td align='left'>
                    <?php echo strtoupper($grupo_info->operacao);?>
                 </td>
                 <td align='center'>
                   <?php
                     if ($pagina_a_exibir=='')
                     ...
                     if($consulta_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/tmovimento/tmovimento_detalhado.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_tipo_movto;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($alteracao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/tmovimento/tmovimento_alteracao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_tipo_movto;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
                   <?php
                     }
                   ?>
                   <?php
                     if($exclusao_perfil!=""){
                   ?>
                     <a href='<?php echo URL;?>/modulos/tmovimento/tmovimento_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&codigo=<?php echo $grupo_info->id_tipo_movto;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
                   <?php
                     }
                   ?>
                 </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/tmovimento/tmovimento_inicial.php";
...
<?php
    ...
    if($_GET[e]=='f'){echo "<script>window.alert('Não foi possível excluir o Tipo de Movimento!')</script>";}
    if($_GET[e]=='r'){echo "<script>window.alert('Não é possível excluir o Tipo de Movimento, pois existe associação com outra tabela!')</script>";}

    if($_GET[i]=='t'){echo "<script>window.alert('Operação realizada com sucesso!')</script>";}
    if($_GET[i]=='f'){echo "<script>window.alert('Não foi possível cadastrar o Tipo de Movimento!')</script>";}

    if($_GET[a]=='t'){echo "<script>window.alert('Operação realizada com sucesso!')</script>";}
    if($_GET[a]=='f'){echo "<script>window.alert('Não foi possível alterar o Tipo de Movimento!')</script>";}
  }
  ////////////////////////////////////////////
