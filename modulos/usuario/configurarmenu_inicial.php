  ...
  ?>
    ...
    <table width="100%" height="100%" border="1" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        {
           $string_query_registros = "select id_item_menu, descricao from item_menu
...
//           echo exit;
      ...
      ?>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <form name="form_inicial" action="./configurarmenu_inicial.php" method="POST" enctype="application/x-www-form-urlencoded">
               ...
               <tr class="opcao_tabela">
                <td valign="middle" width="60%">
                        ...
                        <select name="pesquisa" style="width:150px;">
                           <option <?php if ($_POST[pesquisa]=='id_item_menu'){echo "selected";}?> value="id_item_menu">Código</option>
                           <option <?php if ($_POST[pesquisa]=='descricao'){echo "selected";}?> value="descricao">Módulo</option>
                        </select>
                ...
                <td valign="middle" width="40%">
                  ...
                  <select name="indice" style="width:150px;">
                    <option <?php if ($_POST[indice]=='id_item_menu'){echo "selected";}?> value="id_item_menu">Código</option>
                    <option <?php if ($_POST[indice]=='descricao'){echo "selected";}?> value="descricao">Módulo</option>
                  </select>
                ...
                <td valign="middle" align="right" width="20%">
                  <?php
                    if($inclusao_perfil!=""){
                  ?>
                      <input type="button" style="font-size: 10px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/usuario/configurarmenu_cadastro.php'">
                  <?php
                    }
                    else{
                  ?>
                      <input type="button" style="font-size: 10px;" name="cadastrar" value="Novo >>" onclick="window.location='<?php echo URL;?>/modulos/usuario/configurarmenu_cadastro.php'" disabled>
                  <?php
                    }
                  ?>
                </td>
    ...
    <?php
      ...
      while ($menu = mysqli_fetch_object($resultado_query_limite))
      {
         ...
         <tr class="linha_tabela" bgcolor='<?php echo $cor_linha;?>' onMouseOver="this.bgColor='#D9ECFF';" onMouseOut="this.bgColor='<?echo $cor_linha; ?>';">
           <td align='left'>
               <?php echo $menu->id_item_menu;?>
           </td>

           <td align='left'>
               <?php echo strtoupper($menu->descricao);?>
           </td>
           ...
           <td align='center'>
             <?php
               ...
               if($consulta_perfil!=""){
             ?>
                <a href='<?php echo URL;?>/modulos/usuario/configurarmenu_detalhe.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&id_item_menu=<?php echo $menu->id_item_menu;?>'><img src="<?php echo URL;?>/imagens/b_search.png" border="0" title="Detalhar Registro"></a>&nbsp&nbsp&nbsp
             <?php
               }
             ?>
             <?php
               if($alteracao_perfil!=""){
             ?>
                <a href='<?php echo URL;?>/modulos/usuario/configurarmenu_edicao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&id_item_menu=<?php echo $menu->id_item_menu;?>'><img src="<?php echo URL;?>/imagens/b_edit.gif" border="0" title="Editar Registro"></a>&nbsp&nbsp&nbsp
             <?php
               }
             ?>
             <?php
               if($exclusao_perfil!=""){
             ?>
                <a href='<?php echo URL;?>/modulos/usuario/configurarmenu_exclusao.php?pagina=<?=$pagina_a_exibir-1?>&pagina_a_exibir=<?=$pagina_a_exibir?>&indice=<?=$_POST['indice']?>&buscar=<?=$_POST['buscar']?>&pesquisa=<?=$_POST['pesquisa']?>&id_item_menu=<?php echo $menu->id_item_menu;?>'><img src="<?php echo URL;?>/imagens/trash.gif" border="0" title="Excluir Registro"></a>
             <?php
               }
             ?>
           </td>
...
<?
                      ...
                      ////////////////////////////////////////
                      $parte_url="/modulos/usuario/configurarmenu_inicial.php";
...
<?php
  ...
  if($_GET[e]=='f'){echo "<script>alert('Não foi possível excluir Item')</script>";}

  if($_GET[i]=='t'){echo "<script>alert('Operação efetuada com sucesso!')</script>";}
  if($_GET[i]=='f'){echo "<script>alert('Não foi possível cadastrar Item')</script>";}

  if($_GET[a]=='t'){echo "<script>alert('Operação efetuada com sucesso!')</script>";}
  if($_GET[a]=='f'){echo "<script>alert('Não foi possível alterar Item')</script>";}
}
////////////////////////////////////////////
