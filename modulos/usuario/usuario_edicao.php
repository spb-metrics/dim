...
*/
  ...
  //  Função...: Tela de edicao de usuario
  //////////////////////////////////////////////////////////////////
    ...
    if($_POST[novo_login]=="t"){
      ...
      if($_POST[salvar_cadastro]=="t"){
        if($_POST[situacao]=="I" && count($_SESSION["LISTA"])>0){
          ...
          mysqli_query($db, $sql);
          erro_sql("Delete Unidade Has Usuário", $db, "");
          if(mysqli_errno($db)!="0"){
            $atualizacao="erro";
          }
          if($_POST[situacao]=="A"){
            ...
            foreach($lista as $linha){
              foreach($linha as $coluna){
                ...
                if($index==(QTDE_COLUNA-5)){
                  ...
                  mysqli_query($db, $sql);
                  erro_sql("Insert Unidade Has Usuário", $db, "");
                  if(mysqli_errno($db)!="0"){
                    $atualizacao="erro";
                  }
                  $index=0;
            ...
            if($_POST[flag]=="t"){
              ...
              if($_GET[id_usuario]=="" && $_GET[v]==""){
                ...
                if(!isset($_GET[v])){
                  ...
                  $res=mysqli_query($db, $sql);
                  erro_sql("Select Unidade Has Usuário Escolhido", $db, "");
                ...
                <td colspan="2" align="right" class="descricao_campo_tabela" height="35">
                  ...
                  <?php
                    if(isset($_GET[id_usuario]) || isset($_POST[id_usuario_atual])){
                      ...
                      $res=mysqli_query($db, $sql);
                      erro_sql("Select Usuário Atual", $db, "");
                      if(mysqli_num_rows($res)>0){
...
<?php
    ...
    if($_GET[v]=='f'){echo "<script>window.alert('Unidade já existe na lista!');</script>";}

    if($_GET[l]=='f'){echo "<script>window.alert('Login já cadastrado!');document.cadastro.login.focus();</script>";}

    if($_GET[s]=='f'){echo "<script>window.alert('Não é possível inativar o usuário, pois existe unidade configurada para esse usuário!');</script>";}
  }
  ////////////////////////////////////////////
