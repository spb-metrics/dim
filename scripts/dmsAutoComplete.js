...
function dmsAutoComplete(elem,divname, id_uni_sis, unid, aplic){
		...
		default:
			...
			if (me.elem.value.length >= me.minLength){
				if (me.ajaxReq != undefined){
                    ...
                    if((aplic=="unidadesuperior")||(aplic=="atencao_continuada")||(aplic=="medvencidos")) {
  					  var id_uni_sis_medicamento=document.getElementById(id_uni_sis).value;
					  var unidade=document.getElementById(unid).value;
					  param+="|" + id_uni_sis_medicamento + "|" + unidade;
		...
		if (me.ajaxReq.readyState == 4){
			...
			if (itCnt > 0){
				for (i=0; i<itCnt; i++){
                    ...
                    /*if(itCnt == 1)
                    {
                       me.changeHighlight('ok');
                       me.acChoose(li.id);
                       me.elem.disabled=true;
                       me.hideDiv();
                       me.elem.disabled=false;
                       return false;
                    } */
