...
try {
	...
	$valores = split("[|]", $_POST[cidade_receita]);
    $destaca = $_POST[cidade_receita];
   ...
   $sql="select c.id_cidade, concat(c.nome,'/',e.uf) as nome
			...
			and concat(c.nome,'/',e.uf) like '%$descricao%'
			order by nome";

   $results=mysqli_query($db, $sql);
   erro_sql("Pesquisa cidade", $db, "");
   ...
   while ($row=mysqli_fetch_object($results)){
		...
		$item->setAttribute('id',$row->id_cidade);
		$texto = $row->nome;
