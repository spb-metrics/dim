...
try {
   ...
   if ($aplicacao == "atencao_continuada")
   {
     $sql="select * from unidade where flg_nivel_superior = 0
           and nome like '$descricao%'
           order by nome";
    }
   
    if ($aplicacao == "consulta")
    {
     $sql="select * from unidade where nome like '$descricao%' order by nome";
    }
