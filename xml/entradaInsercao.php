...
//error_reporting(E_ALL);
  ...
  $msg_erro="estoque|NO|Não foi possível cadastrar entrada de material!";
  $atualizacao="";
  ...
  if(mysqli_errno($db)!="0"){
    ...
    if(mysqli_num_rows($result)>0){
      ...
      $sql.="(tipo_movto_id_tipo_movto, usuario_id_usuario, unidade_id_unidade, num_documento,
              data_movto, data_incl, num_controle) ";
      $sql.="values ('2', '$usuario', '$unidade', '$numero', '$data', '$data', '$chave_unica')";
      mysqli_query($db, $sql);
      if(mysqli_errno($db)!="0"){
        $atualizacao="erro";
        $msg=$msg_erro;
      ...
      $sql.="where tipo_movto_id_tipo_movto='2' and usuario_id_usuario='$usuario' and ";
      ...
      for($i=0; $i<count($valores); $i++){
        ...
        $sql.="values ('$chave->id_movto_geral', '" . $valores[$i][0] . "', '" . $valores[$i][1] . "', '" . strtoupper($valores[$i][2]) . "', '" . $valores[$i][3] . "', '" . $valores[$i][4] . "')";
        mysqli_query($db, $sql);
        if(mysqli_errno($db)!="0"){
          $atualizacao="erro";
          $msg=$msg_erro;
        ...
        $sql.="where fabricante_id_fabricante='" . $valores[$i][1] . "' and material_id_material='" . $valores[$i][0] . "' ";
        ...
        $sql="select * from estoque where material_id_material='" . $valores[$i][0] . "' and unidade_id_unidade='$unidade'";
        $res=mysqli_query($db, $sql);
        if(mysqli_errno($db)!="0"){
          $atualizacao="erro";
          $msg=$msg_erro;
        ...
        if(mysqli_num_rows($res)>0){
          ...
          $sql.="and material_id_material='" . $valores[$i][0] . "' and lote='" . $valores[$i][2] . "' and ";
          $sql.="flg_bloqueado='S'";
          $res=mysqli_query($db, $sql);
          if(mysqli_errno($db)!="0"){
            $atualizacao="erro";
          ...
          if(mysqli_num_rows($res)>0){
            ...
            $sql.="values ('" . $valores[$i][1] . "', '" . $valores[$i][0] . "', '$unidade' ,'" . strtoupper($valores[$i][2]) . "', '" . $valores[$i][3] . "', '" . $valores[$i][4] . "', '$data', '$usuario', 'S')";
          }
          //nao existe material/fabricante/lote bloqueado para alguma unidade
          else{
            $sql="insert into estoque ";
            $sql.="(fabricante_id_fabricante, material_id_material, unidade_id_unidade, lote, validade, quantidade, data_incl, usua_incl, flg_bloqueado) ";
            $sql.="values ('" . $valores[$i][1] . "', '" . $valores[$i][0] . "', '$unidade' ,'" . strtoupper($valores[$i][2]) . "', '" . $valores[$i][3] . "', '" . $valores[$i][4] . "', '$data', '$usuario', '')";
          }
        }
        mysqli_query($db, $sql);
        if(mysqli_errno($db)!="0"){
        ...
        if(mysqli_num_rows($res)>0){
          ...
          $sql="select * from tipo_movto where id_tipo_movto='2'";
          $res=mysqli_query($db, $sql);
          if(mysqli_errno($db)!="0"){
            $atualizacao="erro";
            $msg=$msg_erro;
          ...
          $history=$mov_info->descricao . " Nº do documento: " . $numero;
          ...
          $sql.="values ('$chave->id_movto_geral', '$unidade', '" . $valores[$i][0] . "', '2', '$saldo_anterior', '" . $valores[$i][4] . "', '$saldo_atual', '$data', '" . strtoupper($history) . "')";
        }
        mysqli_query($db, $sql);
        if(mysqli_errno($db)!="0"){
          $atualizacao="erro";
  ...
  if($msg==""){
    $msg="estoque|OK|" . $chave->id_movto_geral."|".$data."|";
  }
  echo $msg;
?>
